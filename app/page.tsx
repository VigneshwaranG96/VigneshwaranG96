...
export default function Home() {
  ...
  return (
    <main className="dark bg-[#171715] text-[#ece8df]">
      <section className="relative min-h-screen overflow-hidden">
        ...
        <div className="absolute right-6 top-6 z-20 flex overflow-hidden rounded-full border border-beige/40 glass">
          {(['EN', 'FI'] as const).map((code) => (
            <button
              key={code}
              onClick={() => setLang(code)}
              className={`px-4 py-2 text-xs tracking-[0.16em] transition ${lang === code ? 'bg-warmWhite text-charcoal' : 'text-warmWhite/90 hover:bg-white/10'}`}
        ...
        >
          <p className="text-xs uppercase tracking-[0.36em] text-beige">{lang === 'EN' ? 'Trillby & Chadwick · Helsinki' : 'Trillby & Chadwick · Helsinki'}</p>
          <h1 className="max-w-3xl font-serif text-5xl leading-tight md:text-7xl">
            {lang === 'EN' ? 'A Nordic Dining Experience in the Heart of Helsinki' : 'Pohjoismainen ruokaelämys Helsingin sydämessä'}
          </h1>
          <a href="#reservation" className="rounded-full border border-beige/60 px-8 py-3 text-sm tracking-[0.2em] transition hover:bg-warmWhite hover:text-charcoal">
            {lang === 'EN' ? 'Reserve a Table' : 'Varaa pöytä'}
          </a>
        </motion.div>
        <motion.div
          animate={{ y: [0, 12, 0] }}
      ...
      <section className="section-shell grid gap-14 md:grid-cols-2">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.7 }}>
          <p className="mb-4 text-xs uppercase tracking-[0.3em] text-gold">{lang === 'EN' ? 'About' : 'Tarina'}</p>
          <h2 className="font-serif text-4xl md:text-5xl">{lang === 'EN' ? 'Crafted by Nordic Seasons' : 'Pohjoisen vuodenaikojen muovaama'}</h2>
        </motion.div>
      ...
      <section id="reservation" className="section-shell grid gap-12 md:grid-cols-2">
        <div>
          ...
          <h2 className="font-serif text-4xl md:text-5xl">{lang === 'EN' ? 'Book Your Evening' : 'Varaa iltasi'}</h2>
        ...
        <form onSubmit={handleReservation} className="glass rounded-2xl border border-beige/40 p-8 shadow-soft">
          <div className="grid gap-4">
            ...
            <select required className="rounded-lg border border-beige/50 bg-transparent px-4 py-3">
              <option value="">{lang === 'EN' ? 'Guests' : 'Vieraat'}</option>
              {[2, 4, 6, 8].map((n) => <option key={n}>{n} {lang === 'EN' ? 'Guests' : 'Vieraat'}</option>)}
            </select>
            <button className="mt-2 rounded-full bg-charcoal px-6 py-3 text-sm tracking-[0.18em] text-warmWhite transition hover:bg-black dark:bg-[#ece8df] dark:text-charcoal">
              {lang === 'EN' ? 'Confirm Reservation' : 'Vahvista varaus'}
            </button>
            {confirmed && <p className="text-sm text-gold">{lang === 'EN' ? 'Reservation request sent. We look forward to welcoming you.' : 'Varauspyyntö lähetetty. Tervetuloa.'}</p>}
          </div>
        </form>
      </section>
